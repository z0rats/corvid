...
class ApikeyStateResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"examples": [{"name": "virustotal", "is_active": True}]},
    )
...
class ApikeyBulkLookupStateResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"examples": [{"name": "virustotal", "bulk_ioc_lookup": True}]},
    )
...
class ApikeyStatusSummary(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "examples": [
                {"total_keys": 25, "configured_keys": 8, "active_keys": 6, "bulk_enabled_keys": 3}
            ]
        },
    )
