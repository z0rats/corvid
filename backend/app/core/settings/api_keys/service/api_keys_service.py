...
async def create_apikey_service(
    db: AsyncSession, apikey_data: ApikeyCreateRequest
) -> ApikeySchema | None:
...
async def delete_apikey_service(db: AsyncSession, name: str) -> DeleteApikeyResponse | None:
    ...
    return DeleteApikeyResponse(
        apikey=ApikeySchema.model_validate(apikey), message="API key deleted successfully"
    )
...
async def update_apikey_active_status(
    db: AsyncSession, name: str, is_active: bool
) -> ApikeySchema | None:
...
async def update_apikey_bulk_lookup_status(
    db: AsyncSession, name: str, bulk_ioc_lookup: bool
) -> ApikeySchema | None:
...
async def upsert_apikey_bulk_lookup_status(
    db: AsyncSession, name: str, bulk_ioc_lookup: bool
) -> ApikeySchema:
...
async def update_apikey_service(
    db: AsyncSession, name: str, apikey_data: ApikeyUpdateRequest
) -> ApikeySchema | None:
...
async def get_all_apikeys_configured_status(db: AsyncSession) -> dict[str, bool]:
    ...
    return {
        apikey.name: bool(apikey.key and apikey.key.strip()) for apikey in await get_apikeys(db)
    }
