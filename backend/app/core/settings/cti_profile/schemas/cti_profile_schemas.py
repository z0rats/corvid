...
class CTISettingsData(BaseModel):
    ...
    profile_name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the CTI profile"
    )
    threat_sources: list[str] = Field(
        default_factory=list, description="List of threat intelligence sources"
    )
    indicators_of_interest: list[str] = Field(
        default_factory=list, description="IOCs of particular interest"
    )
    severity_threshold: str = Field(
        default="medium", description="Minimum severity level for alerts"
    )
