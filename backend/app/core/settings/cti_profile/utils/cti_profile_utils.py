...
def validate_ioc_format(ioc_value: str, ioc_type: str) -> bool:
    ...
    validation_patterns = {
        "ip_addresses": r"^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$",  # noqa: E501
        "domain_names": r"^[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?)*$",  # noqa: E501
...
def merge_settings_with_defaults(
    user_settings: dict[str, Any], default_settings: dict[str, Any]
) -> dict[str, Any]:
    """
...
def calculate_threat_score(
    indicators: list[str], severity_weights: dict[str, float] | None = None
) -> float:
    """
...
def format_settings_for_display(settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if "auto_enrichment" in formatted:
        formatted["auto_enrichment_display"] = (
            "Enabled" if formatted["auto_enrichment"] else "Disabled"
        )
