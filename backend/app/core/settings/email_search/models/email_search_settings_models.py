...
class EmailSearchConfig(Base, PypiVersionCheckMixin, TimestampMixin):
    ...
    max_concurrency: Mapped[int] = mapped_column(
        Integer, default=MAX_CONCURRENCY_DEFAULT, comment="Max number of providers checked in parallel"
    )
    proxy_url: Mapped[str | None] = mapped_column(String(500), comment="Optional HTTP(S) proxy URL for provider checks")
    use_tor: Mapped[bool] = mapped_column(Boolean, default=USE_TOR_DEFAULT, comment="Whether to route checks through Tor")
    enable_smtp_checks: Mapped[bool] = mapped_column(
        Boolean, default=ENABLE_SMTP_CHECKS_DEFAULT, comment="Whether to run SMTP-based mailbox existence checks"
    )
    enable_headless_checks: Mapped[bool] = mapped_column(
        Boolean, default=ENABLE_HEADLESS_CHECKS_DEFAULT,
