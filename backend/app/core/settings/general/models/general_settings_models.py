...
class GeneralSettings(Base, TimestampMixin):
    ...
    language: Mapped[str] = mapped_column(
        String(LANGUAGE_MAX_LENGTH), default=get_default_language(), comment="UI language code (e.g. 'en', 'ru')"
    )
    ...
    start_screen: Mapped[str] = mapped_column(
        String(START_SCREEN_MAX_LENGTH), default=get_default_start_screen(),
        comment="Which screen the app opens on startup",
    )
    always_tiles: Mapped[bool] = mapped_column(
        default=get_default_always_tiles(), comment="Whether to always use tile layout instead of tabs"
    )
