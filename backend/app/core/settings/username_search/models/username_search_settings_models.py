...
class UsernameSearchConfig(Base, PypiVersionCheckMixin, TimestampMixin):
    ...
    top_sites_count: Mapped[int] = mapped_column(
        Integer, default=TOP_SITES_COUNT_DEFAULT, comment="Max number of top sites to scan (0 = scan all sites)"
    )
    proxy_url: Mapped[str | None] = mapped_column(String(500), comment="Optional HTTP(S) proxy URL for site checks")
    auto_update_db_enabled: Mapped[bool] = mapped_column(
        Boolean, default=AUTO_UPDATE_DB_ENABLED_DEFAULT,
    ...
    auto_update_interval_hours: Mapped[int] = mapped_column(
        Integer, default=AUTO_UPDATE_INTERVAL_HOURS_DEFAULT, comment="Hours between automatic site-database refreshes"
    )
