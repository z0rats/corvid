...
class DorkRunRequest(BaseModel):
    ...
    target: str = Field(
        ..., min_length=1, max_length=255, description="Domain, username, or email to dork against"
    )
    target_type: TargetType = Field(
        ..., description="Type of the target, used to select applicable templates"
    )
    engine: EngineName = Field(default="duckduckgo", description="Search engine to query")
    template_keys: list[str] | None = Field(
        default=None,
        description=(
            "Subset of template keys to run; omit to run all templates applicable to target_type"
        ),
    )
...
class DorkRunResponse(BaseModel):
    ...
    errors: list[str] = Field(
        default_factory=list, description="Per-query failures (e.g. blocked/CAPTCHA'd), non-fatal"
    )
