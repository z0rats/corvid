...
def check_return_path_mismatch(from_addresses: list[str], return_path: list[str]) -> EmailWarning:
    ...
    if from_addresses != return_path:
        return _make_warning(
            WarningLevel.RED, "'From' and 'Return-Path' fields do not match", message
        )
    ...
    if reply_to and from_addresses and reply_to != from_addresses:
        return [
            _make_warning(
                WarningLevel.AMBER,
                "Reply-To mismatch",
                f"Reply-To address ({reply_to[0]}) doesn't match From address "
                f"({from_addresses[0]})",
            )
        ]
    return []
