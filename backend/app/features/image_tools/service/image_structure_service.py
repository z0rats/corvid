...
def _estimate_quality(values: list[int], standard: list[int]) -> int | None:
    ...
    ratios = [v * 100 / s for v, s in zip(values, standard) if s]
    if not ratios:
        return None
...
def _derive_chroma_subsampling(components: list[FrameComponent]) -> str:
    ...
    if (cb.horizontal_sampling, cb.vertical_sampling) != (cr.horizontal_sampling, cr.vertical_sampling):
        return "custom"
...
def _parse_frame(markers: list[RawMarker]) -> FrameInfo | None:
    for marker in markers:
        ...
        for _ in range(num_components):
            ...
            components.append(FrameComponent(
        ...
        kind = "Baseline" if marker.code == 0xC0 else "Progressive" if is_progressive else "Extended Sequential" if marker.code == 0xC1 else marker_name(marker.code)
...
def analyze_jpeg_structure(filename: str, data: bytes) -> ImageStructureResponse:
    ...
    logger.info(
        ...
        filename, len(markers), len(quantization_tables), len(huffman_tables), overall_quality_estimate,
    )
