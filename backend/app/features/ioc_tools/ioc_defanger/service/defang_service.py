...
from app.features.ioc_tools.ioc_defanger.schemas.defang_schemas import DefangResponse, ProcessedIOC
...
def process_iocs_with_type_detection(text: str, operation: str) -> list[ProcessedIOC]:
    """
...
def process_iocs_without_type_detection(iocs: list[str], processor_func) -> list[ProcessedIOC]:
    """
...
def batch_process_iocs(text: str, operation: str = "defang") -> DefangResponse:
    ...
    try:
        ...
        logger.info(
            "Batch processing completed: %s processed, %s changed",
        ...
        raise RuntimeError(f"Failed to process IOCs: {str(e)}") from e
