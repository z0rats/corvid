...
IOC_PATTERNS = {
    "ips": r"\b(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])(?:\.(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3}\b",  # noqa: E501
...
COMPILED_PATTERNS = {name: re.compile(pattern) for name, pattern in IOC_PATTERNS.items()}
...
def calculate_extraction_statistics(
    ...
    total_unique = sum(
        len(iocs) for ioc_type, iocs in unique_iocs.items() if ioc_type != "domains"
...
def process_content_for_iocs(content: str) -> dict[str, list[str]]:
    ...
    """
    extracted_iocs: dict[str, list[str]] = {ioc_type: [] for ioc_type in IOC_PATTERNS.keys()}
...
def extract_iocs(content: str) -> ExtractionResponse:
    ...
    try:
        ...
        statistics = calculate_extraction_statistics(extracted_iocs, unique_iocs, filtered_domains)
        ...
        logger.info(
            "IOC extraction completed successfully. Found %s unique IOCs",
        ...
        raise RuntimeError(f"Failed to extract IOCs: {str(e)}") from e


def extract_iocs_from_file_content(
    file_content: str, filename: str | None = None
) -> ExtractionResponse:
    """
