...
async def bulk_ioc_lookup(
    ...
    """
    logger.info(
        "Starting bulk lookup for %s IOCs across %s services",
        len(bulk_request.iocs),
        len(bulk_request.services),
    )

    if not bulk_request.iocs:
        raise AppHTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No IOCs provided",
            error_code="BULK_LOOKUP_NO_IOCS",
        )

    if not bulk_request.services:
        raise AppHTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No services specified",
