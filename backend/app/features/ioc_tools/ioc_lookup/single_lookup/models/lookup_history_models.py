...
class SingleLookupSearch(Base):
    ...
    ioc: Mapped[str] = mapped_column(
        String(2000), index=True, comment="The IOC value that was looked up"
    )
    ioc_type: Mapped[str] = mapped_column(
        String(20), index=True, comment="IOC type (e.g. ip, domain, hash, address)"
    )
...
class SingleLookupResult(Base):
    ...
    search_id: Mapped[int] = mapped_column(
        ForeignKey("single_lookup_searches.id", ondelete="CASCADE"),
        index=True,
        comment="Owning SingleLookupSearch.id",
    )
    service_key: Mapped[str] = mapped_column(
        String(100), comment="Internal identifier of the queried service"
    )
    service_name: Mapped[str] = mapped_column(
        String(200), comment="Human-readable name of the queried service"
    )
    status: Mapped[str] = mapped_column(String(20), comment="found, not_found, or error")
    summary: Mapped[str] = mapped_column(
        String(500), comment="Short human-readable summary of the result"
    )
    tlp: Mapped[str] = mapped_column(
        String(20), comment="Traffic Light Protocol label for this result"
    )
