...
IOC_TYPE_PATTERNS = {
    ...
    IOC_TYPES["IPV4"]: re.compile(
        r"^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$"
    ),
    IOC_TYPES["IPV6"]: re.compile(
        r"(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))",
        re.IGNORECASE,
    ),
    IOC_TYPES["URL"]: re.compile(
        r"^(?:(?:https?|ftp):\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:[a-z\u00a1-\uffff0-9](?:[a-z\u00a1-\uffff0-9-]*[a-z\u00a1-\uffff0-9])?)(?:\.[a-z\u00a1-\uffff0-9](?:[a-z\u00a1-\uffff0-9-]*[a-z\u00a1-\uffff0-9])?)*(?:\.(?:[a-z\u00a1-\uffff]{2,}))\.?)(?::\d{2,5})?(?:[/?#]\S*)?$",
        re.IGNORECASE,
    ),
    IOC_TYPES["DOMAIN"]: re.compile(
        r"^(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,63}$"
    ),
    IOC_TYPES["EMAIL"]: re.compile(
    ...
    IOC_TYPES["BITCOIN_ADDRESS"]: re.compile(
        r"^(1[a-zA-Z0-9]{25,34}|3[a-zA-Z0-9]{25,34}|bc1[a-zA-HJ-NP-Z0-9]{25,90})$"
    ),
    IOC_TYPES["TRON_ADDRESS"]: re.compile(r"^T[a-zA-Z0-9]{33}$"),
    IOC_TYPES["XRP_ADDRESS"]: re.compile(r"^r[a-zA-Z0-9]{24,34}$"),
    IOC_TYPES["DOGECOIN_ADDRESS"]: re.compile(r"^D[a-zA-Z0-9]{25,33}$"),
    IOC_TYPES["LITECOIN_ADDRESS"]: re.compile(r"^L[a-zA-Z0-9]{25,34}$"),
    IOC_TYPES["STELLAR_ADDRESS"]: re.compile(r"^[GS][A-Z2-7]{54,58}$"),
    IOC_TYPES["BINANCE_CHAIN_ADDRESS"]: re.compile(r"^bnb1[a-z0-9]{38}$"),
    IOC_TYPES["LISK_ADDRESS"]: re.compile(r"^[0-9]{1,20}L$"),
    IOC_TYPES["CARDANO_ADDRESS"]: re.compile(r"^Ddz[a-zA-Z0-9]{90,110}$"),
}
...
def determine_ioc_type(ioc: str) -> str:
    ...
    if IOC_TYPE_PATTERNS[IOC_TYPES["TRON_ADDRESS"]].match(ioc):
        return IOC_TYPES["TRON_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["XRP_ADDRESS"]].match(ioc):
        return IOC_TYPES["XRP_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["DOGECOIN_ADDRESS"]].match(ioc):
        return IOC_TYPES["DOGECOIN_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["CARDANO_ADDRESS"]].match(ioc):
        return IOC_TYPES["CARDANO_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["LITECOIN_ADDRESS"]].match(ioc):
        return IOC_TYPES["LITECOIN_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["STELLAR_ADDRESS"]].match(ioc):
        return IOC_TYPES["STELLAR_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["BINANCE_CHAIN_ADDRESS"]].match(ioc):
        return IOC_TYPES["BINANCE_CHAIN_ADDRESS"]
    if IOC_TYPE_PATTERNS[IOC_TYPES["LISK_ADDRESS"]].match(ioc):
        return IOC_TYPES["LISK_ADDRESS"]
