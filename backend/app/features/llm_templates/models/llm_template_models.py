...
class AITemplate(Base, TimestampMixin):
    ...
    title: Mapped[str] = mapped_column(String(200), comment="Human-readable template title")
    description: Mapped[str | None] = mapped_column(
        Text, comment="Detailed description of the template's purpose"
    )
    example_input_output: Mapped[str | None] = mapped_column(
        Text, comment="Example showing expected input and output format"
    )
    ai_agent_role: Mapped[str] = mapped_column(
        Text, comment="System prompt defining the AI agent's role and persona"
    )
    ai_agent_task: Mapped[str] = mapped_column(
        Text, comment="Task prompt with instructions for the AI agent"
    )
    payload_fields: Mapped[list[Any]] = mapped_column(
        JSON, comment="JSON array defining required input fields for the template"
    )
    static_contexts: Mapped[list[Any] | None] = mapped_column(
        JSON, default=list, comment="JSON array of static context information to include in prompts"
    )
    web_contexts: Mapped[list[Any] | None] = mapped_column(
        JSON, default=list, comment="JSON array of web URLs to fetch and include as context"
    )
    is_public: Mapped[bool] = mapped_column(
        default=False, comment="Whether the template is publicly accessible"
    )
    user_id: Mapped[str | None] = mapped_column(
        String(100), comment="ID of the user who created the template"
    )
    order_number: Mapped[int | None] = mapped_column(
        Integer, default=0, comment="Display order for template listing"
    )
    temperature: Mapped[float | None] = mapped_column(
        Float, default=0.7, comment="LLM temperature setting (0.0-1.0) for response randomness"
    )
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and bool(self.ai_agent_role and self.ai_agent_role.strip())
            and bool(self.ai_agent_task and self.ai_agent_task.strip())
            and self.payload_fields is not None
        )

    @hybrid_property
    def has_contexts(self) -> bool:
        """Check if the template has any context configurations."""
        return (self.static_contexts and len(self.static_contexts) > 0) or (
