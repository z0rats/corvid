...
class PayloadField(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError(
                "Field name must contain only alphanumeric characters, hyphens, and underscores"
            )
        return v
...
class AITemplateBase(BaseModel):
    ...
    ai_agent_role: str = Field(
        ..., description="AI agent role/system prompt", min_length=1, max_length=2000
    )
    ai_agent_task: str = Field(
        ..., description="AI agent task/user prompt", min_length=1, max_length=5000
    )
    payload_fields: list[dict[str, Any]] = Field(
        default_factory=list, description="Template payload fields"
    )
    static_contexts: list[dict[str, Any]] = Field(
        default_factory=list, description="Static contexts"
    )
    ...
    model: str | None = Field(
        None, description="LLM model to use (null = use configured default)", max_length=100
    )
    ...
    def validate_payload_fields(cls, v: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for field in v:
            ...
            try:
                ...
                raise ValueError(f'Invalid payload field "{name}": {e}') from e
        return v
    ...
    def validate_static_contexts(cls, v: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for i, context in enumerate(v or []):
            ...
            try:
                ...
                raise ValueError(f"Invalid static context {i}: {e}") from e
        return v or []
    ...
    def validate_web_contexts(cls, v: list[dict[str, Any]]) -> list[dict[str, Any]]:
        for i, context in enumerate(v or []):
            ...
            try:
                ...
                raise ValueError(f"Invalid web context {i}: {e}") from e
        return v or []
...
class AITemplateExecute(BaseModel):
    ...
    override_temperature: float | None = Field(
        None, description="Override temperature", ge=0.0, le=1.0
    )
...
class PromptEngineerRequest(BaseModel):
    ...
    model_id: str | None = Field(
        None, description="Model to use for prompt engineering (null = use configured default)"
    )
...
class CategoryDeleteRequest(BaseModel):
    ...
    action: str = Field(
        ...,
        description="Action for templates: move_to_default or delete_templates",
        pattern="^(move_to_default|delete_templates)$",
    )
