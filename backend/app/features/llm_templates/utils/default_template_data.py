...
},
{
  ...
  "description": "Extract tactics, techniques, and procedures (TTPs) from threat intelligence reports or news articles and map them to the MITRE ATT&CK framework. Includes technical observables like IOCs, tools, and infrastructure.",
  "example_input_output": "**Input:**\nA news article describing a spear-phishing campaign that delivers a malicious Word document via email. The document exploits CVE-2023-23397 to execute a PowerShell script that downloads Cobalt Strike from C2 server 198.51.100.23. The attackers use scheduled tasks for persistence and exfiltrate data over DNS tunneling.\n\n**Output:**\n\n# Initial Access\n\n## T1566.001 - Phishing: Spearphishing Attachment\n\n### Behavior\nAttackers delivered a malicious Word document as an email attachment to targeted employees.\n\n### Artifacts\n\n#### Network Artifacts\n- Inbound emails from spoofed sender addresses\n\n#### File Artifacts\n- Malicious .docx file with embedded exploit\n\n#### CVEs\n- CVE-2023-23397 — Exploited vulnerability in Microsoft Outlook for initial code execution\n\n---\n\n# Execution\n\n## T1059.001 - Command and Scripting Interpreter: PowerShell\n\n### Behavior\nAfter exploitation, a PowerShell script was executed to download and run the Cobalt Strike beacon payload.\n\n### Artifacts\n\n#### Network Artifacts\n- Outbound HTTP connection to 198.51.100.23 to download Cobalt Strike payload\n\n#### File Artifacts\n- PowerShell script (dropper)\n- Cobalt Strike beacon DLL\n\n#### Tools\n- PowerShell\n- Cobalt Strike\n\n---\n\n# Persistence\n\n## T1053.005 - Scheduled Task/Job: Scheduled Task\n\n### Behavior\nScheduled tasks were created to ensure the Cobalt Strike beacon survives reboots.\n\n### Artifacts\n\n#### Host Artifacts\n- Windows scheduled task entries\n- Registry modifications for task persistence\n\n---\n\n# Command and Control\n\n## T1071.004 - Application Layer Protocol: DNS\n\n### Behavior\nDNS tunneling was used for C2 communication between the compromised host and attacker infrastructure.\n\n### Artifacts\n\n#### Network Artifacts\n- Anomalous DNS query patterns (high volume, long subdomain labels)\n- C2 server: 198.51.100.23\n\n---\n\n# Exfiltration\n\n## T1048 - Exfiltration Over Alternative Protocol\n\n### Behavior\nSensitive data was exfiltrated from the network using DNS tunneling.\n\n### Artifacts\n\n#### Network Artifacts\n- Encoded data in DNS TXT record queries\n- Unusually large DNS response payloads\n\n---\n\n# Attack Flow\nSpearphishing email → Malicious Word doc → CVE-2023-23397 exploit → PowerShell dropper → Cobalt Strike beacon → Scheduled task persistence → DNS tunneling exfiltration",
  "ai_agent_role": "You are an expert threat intelligence analyst with deep knowledge of the MITRE ATT&CK framework, adversary tradecraft, and technical indicator analysis. You produce structured, actionable intelligence products from raw reporting.",
  "ai_agent_task": "Analyze the following threat intelligence report or news article. Extract all tactics, techniques, and procedures (TTPs) and map each to the MITRE ATT&CK framework with the correct technique ID.\n\nStructure your output strictly as follows, sorted by MITRE ATT&CK tactic order (Reconnaissance → Initial Access → Execution → Persistence → Privilege Escalation → Defense Evasion → Credential Access → Discovery → Lateral Movement → Collection → Command and Control → Exfiltration → Impact):\n\nFor each tactic found, output:\n\n# Tactic Name\n\n## Txxxx.xxx - Technique Name\n\n### Behavior\nDescribe the observed adversary behavior.\n\n### Artifacts\n\n#### Network Artifacts\n- List any network-level indicators (IPs, domains, URLs, DNS patterns, traffic anomalies)\n\n#### File Artifacts\n- List any file-level indicators (file names, hashes, file paths, malware samples)\n\n#### Host Artifacts\n- List any host-level indicators (registry keys, scheduled tasks, services, processes)\n\n#### Tools\n- List any tools, frameworks, or malware families used\n\n#### CVEs\n- List any exploited vulnerabilities\n\nOmit any artifact subsection that has no entries. Separate each tactic with a horizontal rule (---).\n\nAfter all tactics, include:\n\n# Attack Flow\nA concise step-by-step attack chain showing how the TTPs connect using → arrows.\n\nBe precise with MITRE ATT&CK IDs (use sub-technique IDs like T1566.001 where applicable). If a mapping is uncertain, note it with \"(estimated)\" next to the ID. Only include TTPs that are explicitly described or strongly implied by the text.\n\n---\n\n<report>\n{{report_text}}\n</report>",
  "payload_fields": [
    {
      "name": "report_text",
      "description": "Paste the threat report, news article, or advisory text to analyze.",
      "required": True
    }
  ],
  "is_public": True
