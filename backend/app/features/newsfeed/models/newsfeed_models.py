...
class NewsfeedSettings(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(255), primary_key=True, comment="Unique feed name, natural key"
    )
    ...
    icon_id: Mapped[str] = mapped_column(
        String,
        default=generate_icon_id,
        comment="Cache-busting id appended to the icon URL on the frontend",
    )
    enabled: Mapped[bool] = mapped_column(
        default=True, comment="Whether this feed is actively fetched"
    )
    ...
    last_error: Mapped[str | None] = mapped_column(
        String(500), comment="Error message from the last failed fetch, if any"
    )
...
class NewsArticle(Base):
    ...
    feedname: Mapped[str] = mapped_column(
        String(255),
        ForeignKey("newsfeed_settings.name", ondelete="CASCADE"),
        index=True,
        comment="Owning feed's name (newsfeed_settings.name)",
    )
    icon: Mapped[str] = mapped_column(
        String(36), comment="Copy of the owning feed's icon filename at fetch time"
    )
    title: Mapped[str] = mapped_column(String(500), comment="Article title")
    summary: Mapped[str] = mapped_column(
        Text,
        comment="RSS description/summary field - no length cap in feed_processing_service.py, "
    ...
    link: Mapped[str] = mapped_column(
        String(2048), unique=True, index=True, comment="Article URL, deduplication key"
    )
    ...
    analysis_result: Mapped[str | None] = mapped_column(
        Text, comment="LLM-generated analysis of the article"
    )
    mitre_attack: Mapped[str | None] = mapped_column(
        Text, comment="LLM-extracted MITRE ATT&CK technique mapping"
    )
    note: Mapped[str | None] = mapped_column(Text, comment="User-entered free-text note")
    tlp: Mapped[str] = mapped_column(
        String, default="TLP:CLEAR", comment="Traffic Light Protocol label"
    )
    read: Mapped[bool] = mapped_column(
        default=False, comment="Whether the user has marked this article as read"
    )
...
class NewsfeedConfig(Base):
    ...
    fetch_interval_minutes: Mapped[int] = mapped_column(
        default=60, comment="Minutes between scheduled feed fetches"
    )
...
class TrendsBlacklistEntry(Base):
    ...
    value: Mapped[str] = mapped_column(
        String(255), index=True, comment="Blacklisted word or IOC value, lowercased"
    )
