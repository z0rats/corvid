...
@router.get(
    ...
    description=(
        "Retrieve news articles from the database. Use the paginated /articles "
...
@router.get(
    ...
    description=(
        "Retrieve news articles with pagination and optional filters for date "
        "range, IOC presence, TLP, and read status"
    ),
)
async def get_paginated_articles_route(
    ...
    has_relevant_iocs: bool | None = Query(
        None, description="Filter for articles with relevant IOCs"
    ),
    has_analysis: bool | None = Query(
        None, description="Filter for articles with analysis results"
    ),
    ...
    return await fetch_paginated_articles(
        ...
        has_iocs=has_iocs,
        has_relevant_iocs=has_relevant_iocs,
...
@router.get(
    ...
    description=(
        "Get a lightweight list of recent article summaries filtered by a relative time window"
    ),
)
...
@router.post(
    ...
    description=(
        "Fetch and store new articles from all enabled feeds, waiting for the "
        "operation to complete before responding"
    ),
)
...
@router.get(
    ...
    description=(
        "Stream LLM analysis of the top 10 articles via Server-Sent Events. "
        "Yields ranking, analysis, and complete events."
    ),
)
