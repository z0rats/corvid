...
class NewsfeedSettingsSchema(BaseModel):
    ...
    last_fetched_at: datetime.datetime | None = Field(
        default=None, description="Timestamp of the most recent fetch attempt"
    )
    last_success_at: datetime.datetime | None = Field(
        default=None, description="Timestamp of the most recent successful fetch"
    )
    last_error: str | None = Field(
        default=None, description="Error from the most recent failed fetch attempt, if any"
    )
...
class NewsArticleSchema(BaseModel):
    ...
    matches: list[str] | None = Field(
        default=None, description="Keyword matches found in the article"
    )
    iocs: dict[str, list[str]] | None = Field(
        default=None, description="Extracted indicators of compromise"
    )
    relevant_iocs: list[str] | None = Field(default=None, description="IOCs flagged as relevant")
    analysis_result: str | None = Field(
        default=None, description="LLM analysis result for this article"
    )
    ...
    tlp: str | None = Field(
        default="TLP:CLEAR", description="Traffic Light Protocol classification"
    )
...
class NewsfeedConfigSchema(BaseModel):
    ...
    background_fetch_enabled: bool = Field(
        ..., description="Whether background feed fetching is active"
    )
    fetch_interval_minutes: int = Field(..., description="Interval between feed fetches in minutes")
    last_fetch_timestamp: datetime.datetime | None = Field(
        default=None, description="Timestamp of the last feed fetch"
    )
    keyword_matching_enabled: bool = Field(
        default=False, description="Whether keyword matching is enabled"
    )
...
class NewsfeedConfigUpdateSchema(BaseModel):
    retention_days: int | None = Field(
        default=None, description="Number of days to retain articles"
    )
    background_fetch_enabled: bool | None = Field(
        default=None, description="Whether background feed fetching is active"
    )
    fetch_interval_minutes: int | None = Field(
        default=None, description="Interval between feed fetches in minutes"
    )
    keyword_matching_enabled: bool | None = Field(
        default=None, description="Whether keyword matching is enabled"
    )
...
class UpdateArticleRequest(BaseModel):
    ...
    tlp: str | None = Field(
        default=None, description="TLP classification (e.g. TLP:CLEAR, TLP:GREEN)"
    )
...
class NewsAnalysisParams(BaseModel):
    ...
    model_id: str | None = Field(
        default=None, description="Model ID to use for analysis (null = use configured default)"
    )
    temperature: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Temperature for generation (0.0-1.0)"
    )
    ...
    analyze_missing_perspectives: bool = Field(
        default=True, description="Include missing perspectives"
    )
...
class MitreTTP(BaseModel):
    ...
    kill_chain_phase: str | None = Field(
        default=None, description="Kill chain phase, e.g. delivery"
    )
    behavior: str = Field(
        default="", description="Description of the observed behavior from the article"
    )
...
class ThreatIntelEnrichment(BaseModel):
    ...
    has_mitre_data: bool = Field(
        default=False, description="False when article has no ATT&CK relevance"
    )
...
class ArticleAnalysisRequest(BaseModel):
    ...
    model_id: str | None = Field(
        default=None, description="LLM model ID to use for analysis (null = use configured default)"
    )
    temperature: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Temperature for generation (0.0–1.0)"
    )
    max_tokens: int = Field(default=2000, ge=1, description="Maximum tokens in the response")
    use_cti_settings: bool = Field(
        default=True, description="Apply CTI profile settings to the analysis"
    )
    force: bool = Field(
        default=False, description="Force re-analysis even if a result already exists"
    )
    mode: AnalysisMode = Field(
        default="all", description="Which analysis to run: all, analysis, or mitre"
    )
...
class AnalysisResultResponse(BaseModel):
    ...
    mitre_attack: dict[str, Any] | None = Field(
        default=None, description="MITRE ATT&CK enrichment data"
    )
    cti_settings_used: bool | None = Field(
        default=None, description="Whether CTI profile settings were applied"
    )
...
class IocEntry(BaseModel):
    ...
    article_ids: list[int] = Field(
        default_factory=list, description="IDs of articles containing this IOC"
    )
...
class CveEntry(BaseModel):
    ...
    count: int = Field(..., description="Number of occurrences")
    article_ids: list[int] = Field(
        default_factory=list, description="IDs of articles mentioning this CVE"
    )
...
class WordFrequencyEntry(BaseModel):
    ...
    count: int = Field(..., description="Number of occurrences")
    article_ids: list[int] = Field(
        default_factory=list, description="IDs of articles containing this word"
    )
...
class ArticleAnalysisResponse(BaseModel):
    ...
    articles_analysis: list[ArticleAnalysisResult] = Field(
...
class BulkArticleRequest(BaseModel):
    ...
    article_ids: list[int] = Field(
        ..., description="List of article IDs", min_length=1, max_length=200
    )
...
class RetentionDaysUpdate(BaseModel):
    ...
    retention_days: int = Field(
        ..., ge=0, description="Number of days to retain articles (0 = unlimited)"
    )
...
class BlacklistEntryCreate(BaseModel):
    ...
    value: str = Field(
        ..., min_length=1, max_length=255, description="Value to blacklist (word or IOC)"
    )
