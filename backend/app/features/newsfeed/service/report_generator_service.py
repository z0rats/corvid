...
RANKING_PROMPT = """
You are a very experienced cyber threat intelligence analyst specialized in analyzing large
amounts of news articles for relevance.
Below is a list of news article headlines. For each headline, consider how relevant it is for
cybersecurity and threat intelligence.
News articles have a higher relevance, if there are more news article headlines about the same
topic and / or if it describes about active threats or vulnerabilities.
Your task:
1. Identify the 10 most relevant articles from the list (if there are fewer than 10, select them
   all).
...
ANALYSIS_PROMPT = """
You are a very experienced cyber threat intelligence analyst specialized in in creating best in
class news article analysis.
...
<Risk Criteria>
High Risk - Immediate, active threats with high confidence—such as zero-day exploits or
unpatched vulnerabilities—that require urgent action to protect critical systems. Severe threats
with potential for significant impact where mitigations exist but may be underutilized,
necessitating prompt response.
Medium Risk - Emerging vulnerabilities or attack trends that could impact operations under
specific conditions, meriting careful monitoring.
Low Risk - Minor issues or outdated threats unlikely to affect core operations, generally
limited in scope or impact.
Informational - Background or analytical content that provides context and insights without
...
async def analyze_top_articles_stream(db: AsyncSession):
    ...
    if not top_articles_ranking:
        yield json.dumps({"type": "ranking", "articles": [], "info": "No ranked articles found."})
        yield json.dumps({"type": "complete", "message": "All analysis done"})
        return

    yield json.dumps({"type": "ranking", "articles": top_articles_ranking})
    ...
    for article_info in top_articles_ranking:
        ...
        yield json.dumps({"type": "analysis", "article_result": result_entry})
