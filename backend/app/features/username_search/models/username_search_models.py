...
class MaigretSearch(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('running', 'completed', 'cancelled', 'failed')", name="ck_maigret_searches_status"
        ),
    )
    ...
    source: Mapped[str] = mapped_column(
        String(30), default="maigret", server_default="maigret", index=True,
        comment="Which tool produced this run: 'maigret' or 'social_analyzer'",
    )
    status: Mapped[str] = mapped_column(
        String(20), default="running", index=True, comment="running, completed, cancelled, or failed"
    )
    total_sites_checked: Mapped[int] = mapped_column(Integer, default=0, comment="Sites checked so far")
    found_count: Mapped[int] = mapped_column(Integer, default=0, comment="Sites where the username was claimed/found")
    error_message: Mapped[str | None] = mapped_column(String(1000), comment="Error detail if status is failed")
    tags: Mapped[list[str] | None] = mapped_column(JSON, comment="Maigret category tags matched for this username")
...
class MaigretSiteResult(Base):
    ...
    search_id: Mapped[int] = mapped_column(
        ForeignKey("maigret_searches.id", ondelete="CASCADE"), index=True, comment="Owning MaigretSearch.id"
    )
    site_name: Mapped[str] = mapped_column(String(200), comment="Site where the username was claimed/found")
    url_user: Mapped[str] = mapped_column(String(2000), comment="Direct URL to the found profile")
    http_status: Mapped[int | None] = mapped_column(Integer, comment="HTTP status code returned by the site check")
    extra: Mapped[dict | None] = mapped_column(
        JSON, comment="Source-specific extras not worth their own columns (e.g. social-analyzer's match rate/title)"
    )
