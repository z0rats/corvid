...
from app.features.username_search.service.db_refresh_service import (
    refresh_database as refresh_maigret_database,
)
...
async def _execute_maigret_db_refresh_job() -> None:
    async with managed_session() as db:
        ...
        site_count = await refresh_maigret_database(
            db, check_interval_hours=config.auto_update_interval_hours
        )
...
async def register_maigret_db_scheduler() -> None:
    ...
    configure_recurring_job(
        MAIGRET_DB_REFRESH_JOB_ID,
        wrap_job_errors("Maigret DB refresh", _execute_maigret_db_refresh_job),
        enabled=config.auto_update_db_enabled,
