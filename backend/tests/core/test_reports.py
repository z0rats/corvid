...
def test_render_html_escapes_malicious_row_values():
    ...
    sections = [
        ReportSection(
            title="Findings", rows=[ReportRow(label="Subject", value="<script>alert(1)</script>")]
        )
    ]
...
def test_render_html_escapes_malicious_section_title_and_report_title():
    ...
    html = render_html(
        "Report <b>title</b>", sections, locale="en", generated_at_label="Generated at"
    )
...
def test_render_html_renders_all_sections_and_rows_in_order():
    sections = [
        ReportSection(
            title="Section A",
            rows=[ReportRow(label="k1", value="v1"), ReportRow(label="k2", value="v2")],
        ),
    ...
    assert (
        html.index("Section A")
        < html.index("k1")
        < html.index("k2")
        < html.index("Section B")
        < html.index("k3")
    )
...
def test_render_html_renders_a_row_with_href_as_a_clickable_link():
    sections = [
        ReportSection(
            title="Findings",
            rows=[ReportRow(label="Source", value="Case #1", href="https://example.com/case/1")],
        )
    ]

    html = render_html("Report", sections, locale="en", generated_at_label="Generated at")

    assert '<a href="https://example.com/case/1">Case #1</a>' in html


def test_render_html_renders_a_row_without_href_as_plain_text():
    sections = [ReportSection(title="Findings", rows=[ReportRow(label="Status", value="Clean")])]

    html = render_html("Report", sections, locale="en", generated_at_label="Generated at")

    assert "<a href" not in html
    assert "<td>Clean</td>" in html


def test_render_html_escapes_a_malicious_href_rather_than_breaking_out_of_the_attribute():
    sections = [
        ReportSection(
            title="Findings",
            rows=[ReportRow(label="Source", value="x", href='"><script>alert(1)</script>')],
        )
    ]
...
def test_render_pdf_produces_non_empty_pdf_bytes():
    html = render_html(
        "Report",
        [ReportSection(title="A", rows=[ReportRow(label="k", value="v")])],
        "en",
        "Generated at",
    )
...
def test_generate_report_html_format_returns_utf8_encoded_bytes():
    ...
    assert content.decode("utf-8") == render_html(
        "Report", sections, locale="en", generated_at_label="Generated at"
    )
