...
class TestErrorHandling:
    def test_a_pbsearchcaptcha_error_raises_a_clean_captcha_error(self, monkeypatch):
        def handler(request):
    ...
    def test_a_pbratelimit_error_raises_a_clean_rate_limit_error(self, monkeypatch):
        def handler(request):
    ...
    def test_an_inline_captcharequired_flag_on_a_200_raises_a_clean_error(self, monkeypatch):
        def handler(request):
    ...
    def test_polling_forever_null_eventually_raises_a_clean_timeout_error(self, monkeypatch):
        def handler(request):
...
class TestSuccessPath:
    def test_full_flow_returns_parsed_result_and_raw_payload(self, monkeypatch):
    ...
    def test_no_matching_search_row_is_a_clean_not_found_result_not_an_error(self, monkeypatch):
        def handler(request):
            body = (request.content or b"").decode()
            if "queryAll" in body:
                return httpx.Response(200, json={"id": "search-job", "captchaRequired": False})
