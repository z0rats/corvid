...
const logger = createLogger('EmailSearchHistory');
...
export default function HistoryList() {
  ...
  const loadRuns = useCallback(async () => {
    try {
      setLoading(true);
      const data = await emailSearchApi.listRuns();
      setRuns(data);
    } catch (err) {
  ...
  const handleDelete = async (id, e) => {
    e.stopPropagation();
    try {
      await emailSearchApi.deleteRun(id);
        ...
        <TableBody>
          {runs.map((run) => (
            <TableRow
              ...
              hover
              sx={{ cursor: 'pointer' }}
              onClick={() => navigate(`/email-search/history/${run.id}`)}
            >
              ...
              <TableCell align="right">
                <IconButton size="small" onClick={() => navigate(`/email-search/history/${run.id}`)}>
